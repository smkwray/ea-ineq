CONFIG_THIS <- if (exists(".__CONFIG_PATH__", inherits = TRUE)) get(".__CONFIG_PATH__", inherits = TRUE) else file.path(getwd(), "config_dflmx_poverty_consumption_baseline.R")
...
DASS_OUT_DIR <- file.path(RESULTS_ROOT, "dass-R", "poverty_consumption_baseline")
STACKED_CSV <- file.path(DASS_OUT_DIR, "stacked_quarterly.csv")
DASS_CONFIG_R <- file.path(ROOT, "dass-R", "config_dass_poverty_consumption_baseline.R")
OUT_DIR <- file.path(RESULTS_ROOT, "dflmx-R", "poverty_consumption_baseline")

# Inherit file layout defaults from full config (paths rewritten to baseline OUT_DIR above).
FULL_PATH <- file.path(DFLMX_ROOT, "config_dflmx_poverty_consumption.R")
...
MANUAL_TREATMENTS <- c("fed_funds", "transfer_composite", "social_security", "ui_benefits", "snap_persons", "household_networth", "credit_composite")
OUTCOME_QEND_COLS <- c("qend__pce_essential_v2_idx", "qend__pce_discretionary_v2_idx", "qend__pce_gap_v2", "qend__pce_eshare_v2")
LP_HORIZONS <- c(1, 2)
...
# Baseline: disable IV/negative-control discovery loops.
RUN_IV_NC_DISCOVERY <- FALSE

# Keep regression checks conservative.
REGRESSION_MAX_FAILS <- FULL_ENV$REGRESSION_MAX_FAILS
REGRESSION_MAX_REAL_SECONDS <- FULL_ENV$REGRESSION_MAX_REAL_SECONDS
REGRESSION_MAX_ATTEMPTS_BY_TREATMENT <- FULL_ENV$REGRESSION_MAX_ATTEMPTS_BY_TREATMENT
