CONFIG_THIS <- if (exists(".__CONFIG_PATH__", inherits = TRUE)) get(".__CONFIG_PATH__", inherits = TRUE) else file.path(getwd(), "config_dflmx_poverty_consumption.R")
...
DASS_OUT_DIR <- file.path(RESULTS_ROOT, "dass-R", "poverty_consumption")
STACKED_CSV <- file.path(DASS_OUT_DIR, "stacked_quarterly.csv")
DASS_CONFIG_R <- file.path(ROOT, "dass-R", "config_dass_poverty_consumption.R")
OUT_DIR <- file.path(RESULTS_ROOT, "dflmx-R", "poverty_consumption")
...
MANUAL_TREATMENTS <- c(
  "fed_funds", "transfer_composite", "social_security", "ui_benefits", "snap_persons",
  "household_networth", "home_equity", "credit_composite", "revolving_credit", "cc_delinquency"
)
OUTCOME_QEND_COLS <- c(
  "qend__pce_essential_v2_idx",
  "qend__pce_discretionary_v2_idx",
  "qend__pce_gap_v2",
  "qend__pce_eshare_v2",
  "qend__pce_fcsu_idx"
)
...
REGRESSION_MAX_ATTEMPTS_BY_TREATMENT <- list(
  fed_funds = 6,
  transfers_total = 6,
  transfer_composite = 6,
  social_security = 6,
  ui_benefits = 6,
  snap_persons = 6,
  household_networth = 6,
  equity_wealth_proxy = 6,
  home_equity = 6,
  fhfa_hpi = 6,
  credit_composite = 6,
  total_credit = 6,
  revolving_credit = 6,
  nonrevolving_credit = 6,
  cc_delinquency = 6,
  top1_wealth_share = 6,
  top10_wealth_share = 6,
  bottom50_wealth_share = 6,
  wealth_share_gap_top10_bottom50 = 6,
  wealth_share_gap_top1_bottom50 = 6,
  gini_income_fred_q = 6,
  median_real_income_fred_q = 6
)
...
RECESSION_STATE_COLUMNS <- c("m__recession_nber__lag001", "d__recession_nber_daily__lag001")
