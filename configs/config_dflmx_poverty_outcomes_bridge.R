CONFIG_THIS <- if (exists(".__CONFIG_PATH__", inherits = TRUE)) get(".__CONFIG_PATH__", inherits = TRUE) else file.path(getwd(), "config_dflmx_poverty_outcomes_bridge.R")
...
DASS_OUT_DIR <- file.path(RESULTS_ROOT, "dass-R", "poverty_outcomes_bridge")
STACKED_CSV <- file.path(DASS_OUT_DIR, "stacked_quarterly.csv")
DASS_CONFIG_R <- file.path(ROOT, "dass-R", "config_dass_poverty_outcomes_bridge.R")
OUT_DIR <- file.path(RESULTS_ROOT, "dflmx-R", "poverty_outcomes_bridge")
...
MANUAL_TREATMENTS <- c("ui_benefits", "transfers_total", "transfer_composite_fp")
OUTCOME_QEND_COLS <- c(
  "qend__poverty_all_q",
